  ...
  function buildArmyDataForPdf() {
    ...
    function collectEquipment(unit, selectedOptionIds) {
      let eq = ensureArray(unit.equipment);
      for (const opt of unit.options || []) {
        if (selectedOptionIds.has(opt.id) && opt.add_equipment) {
          eq = eq.concat(ensureArray(opt.add_equipment));
        }
      }
      return eq;
    }
  ...
  function renderUnitEquipment(unit, selectedOptionIds) {
    ...
    let eq = [];
    eq = eq.concat(ensureArray(unit.equipment));
  ...
  function renderConfigPanel(existingEntry = null) {
    ...
    let defOpts = [];
    if (unit.equipment) defOpts = defOpts.concat(unit.equipment);
    if (unit.mount) {
    ...
    mainBtn.onclick = () => {
      ...
      if (unit.equipment) preselectedOptions = preselectedOptions.concat( unit.equipment );
