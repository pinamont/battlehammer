...
def parse_additional_rules(add_text):
    ...
    if m:
        items = re.split(r" o | e |, ", m.group(1))
        rules["only_for_army"] = [slugify(x.strip()) for x in items]
        return rules
    ...
    if m:
        items = re.split(r" o | e |, ", m.group(1))
        rules["not_for_army"] = [slugify(x.strip()) for x in items]
        return rules
...
# Parser principale
# ------------------------------------------------------------
def parse_magic_items(tex_content):
    items = []
...
if __name__ == "__main__":
    ...
    items = parse_magic_items(tex)

    output = {
        "magic_items": items
    }

    with open("data/magic_items/magic_items.json", "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)

    print("Creato magic_items.json con", len(items), "oggetti.")
